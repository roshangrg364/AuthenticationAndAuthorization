...
namespace EmailModule.Service
{
    public class EmailTemplateService : EmailTemplateServiceInterface
    {
        ...
        public async Task Create(string type, string template)
        {
            ...
            await _templateRepo.InsertAsync(emailTemplate).ConfigureAwait(false);
        }

        public async Task Update(long id, string type, string template)
        {
            var emailTemplate = await _templateRepo.GetById(id).ConfigureAwait(false) ?? throw new Exception("template not found");
            await ValidateTemplateType(type, emailTemplate);
            emailTemplate.Update(type, template);
            await _templateRepo.UpdateAsync(emailTemplate).ConfigureAwait(false);
        }
